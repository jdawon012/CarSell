...
@Controller
@RequestMapping("/oldcarList")
public class oldCarListController {
	...
	// 리스트 화면
	@GetMapping
	public String oldcarList(Model model, HttpServletRequest request) {
		...
		List<String> condition = Arrays.asList("old", "sellrdy");
		
		List<Car> list = service.carListOld(startNum, endNum,condition);
		...
		return "car/oldCarList";
	}
	
	@PostMapping
	public String oldcarListSearch(Model model, HttpServletRequest request) {
	   ...
	   int pg = 1;
		...
		// 총개수 구하기
		int total = service.oldcarListCount(minPrice, maxPrice, makers, sizes, fuels, condition,makersNullCheck, sizesNullCheck,fuelsNullCheck); // 총 글수;
